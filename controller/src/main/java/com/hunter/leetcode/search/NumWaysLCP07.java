...
public class NumWaysLCP07 {
    ...
    public static int numWays(int n, int[][] relation, int k) {
        ...
        return getSource(relation, target, 1, k);
    }

    private static int getSource(int[][] relation, int target, int deepth, int k) {

        for (int i = 0; i < relation.length; i++) {
            if (relation[i][1] == target) {
                deepth++;
                return getSource(relation, relation[i][0], deepth, k);
            }
            if (relation[i][0] == 0 && k == deepth) {
                return ++count;
            }
            if (deepth > k) {
                return 0;
